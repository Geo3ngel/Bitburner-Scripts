...
/**
 * Gets the port number assigned to a service.
 */
export function getServicePort(service_name) {
	return SERVICE_PORTS_MAP[service_name];
}
/**
 * Takes the port number and returns the service it's associated with.
 */
export function getServiceByPort(port) {
	for (let [key, val] of SERVICE_PORTS_MAP.entries()) {
		if (val === port) {
			return key;
		}
	}
 ...
 * ##########################################
 */
export function getCoreBonus(ns) {
